...
# Identificação automática de bairros e polos de saúde em Belém
BAIRROS_BELEM = {
...
def gerar_carta_apresentacao(vaga: Job, perfil_kws: list) -> str:
  ...
  return f"""Prezada equipa de Recrutamento e Seleção do {vaga.hospital_or_company},

Venho por meio deste manifestar meu forte interesse na vaga de {vaga.title}.

Possuo sólida formação técnica e dedicação à prática assistencial humanizada, ética e segura. O meu perfil alinha-se às competências necessárias para a função, com interesse e conhecimentos consolidados nas áreas de {areas}, além do registo ativo junto ao COREN-PA.
...
# --- CSS COM CONTRASTE APURADO E TEMA HELLO KITTY ---
st.markdown(
    """
<style>
    .stApp {
...
busca_livre = st.sidebar.text_input(
    "🔍 Busca rápida", placeholder="Ex: Ophir, UTI, Unimed, Pediatria..."
)
...
st.sidebar.markdown("---")
# Biscoito da Sorte Diário na Barra Lateral
biscoitos = [
...
if st.sidebar.button("🔄 Atualizar Vagas da Web Agora"):
  with st.spinner("Buscando oportunidades fresquinhas em Belém..."):
    ...
    with Session(engine) as session:
      for v in novas:
        if not session.exec(
            select(Job).where(Job.url_apply == v["url_apply"])
        ).first():
          session.add(Job(**v))
          qtd += 1
      session.commit()
      ...
      if st.form_submit_button("Cadastrar"):
        if "@" in em:
          with Session(engine) as s:
            if not s.exec(
                select(UserSubscription).where(UserSubscription.email == em)
            ).first():
              s.add(UserSubscription(email=em))
              s.commit()
          st.success("E-mail registado!")
        else:
...
with st.expander("📄 Anexar Meu Currículo para Match Inteligente"):
  ...
  with col_c2:
    txt_in = st.text_area(
        "Ou liste suas especialidades (ex: UTI, Pediatria, Centro Cirúrgico)",
        height=90,
    )
...
with Session(engine) as session:
  ...
  if p and p.skills_keywords:
    user_keywords = [
        k.strip() for k in p.skills_keywords.split(",") if k.strip()
    ]
...
# --- SISTEMA DE ABAS ---
tab_vagas, tab_candidaturas, tab_dicas = st.tabs(
    ["🌸 Vagas Disponíveis", "📋 Minhas Candidaturas", "💡 Guia Salarial & Apoio"]
)
...
with tab_vagas:
  ...
  if user_keywords:
    ...
    st.markdown(
        f"🌟 *Vagas ordenadas pelo seu perfil:* `{' | '.join(user_keywords).upper()}`"
    )

  st.markdown(
      f"<h4 style='color: #AD1457;'>🩺 Oportunidades Encontradas:"
      f" <b>{len(rankeadas)}</b></h4>",
      unsafe_allow_html=True,
  )
  ...
  if not rankeadas:
    ...
    for vaga, score in rankeadas:
      link = (
          vaga.url_apply
          if vaga.url_apply.startswith("http")
          else f"https://{vaga.url_apply}"
      )
      bairro = detectar_bairro(
          f"{vaga.location} {vaga.hospital_or_company} {vaga.description}"
      )
      gmaps_url = f"https://www.google.com/maps/search/{urllib.parse.quote(f'{vaga.hospital_or_company} {bairro} Belém')}"

      badge_match = (
          f'<span class="badge-match">✨ Compatibilidade: {score}%</span><br>'
          if score > 0
          else ""
      )
      # Selo Piso Salarial Nacional
      badge_piso = (
          '<span class="badge-piso">⚖️ Piso Salarial Nacional'
          " Assegurado</span>"
      )
          ...
          f"""
            <div class="job-card">
                ...
                <div class="job-meta">
                    ...
                    <b>Status:</b> 🏷️ {vaga.status or 'Disponível'}
                </div>
                <div>
      ...
      # Botões de Ação e Gerador de Apresentação
...
with col_rod1:
  st.markdown(
      "<div style='color: #AD1457; font-size: 0.85rem;'>Feito sob medida com"
      " todo o amor do mundo para a futura melhor enfermeira de Belém 💕"
      " 🐾</div>",
      unsafe_allow_html=True,
  )
with col_rod2:
  with st.popover("🎀 Clique aqui"):
    st.markdown("""
        **Para o meu amor:**
        ...
        estou sempre aqui a torcer por você.

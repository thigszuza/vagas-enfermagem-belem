...
st.set_page_config(
    page_title="Portal de Vagas Saúde & Biomedicina 💕",
    page_icon="🎀",
    layout="wide",
...
def calcular_match(vaga: Job, perfil_keywords: list) -> int:
  ...
  texto_vaga = normalizar_texto(
      f"{vaga.title} {vaga.description} {vaga.specialty}"
      f" {vaga.hospital_or_company}"
  )
...
def simular_analise_ia_thiago(vaga: Job, perfil_kws: list) -> str:
  # Análise afetiva e analítica da Hello Kitty com o tom carinhoso do Thiago
  match_perc = calcular_match(vaga, perfil_kws)
  pontos_fortes = [kw.upper() for kw in perfil_kws if kw in normalizar_texto(vaga.description + vaga.title)]
  
  msg = f"🐾 **Oi meu amor! Aqui é a Hello Kitty falando em nome do Thiago!** 💕\n\n"
  msg += f"Analisei cada detalhe da vaga de **{vaga.title}** no **{vaga.hospital_or_company}**:\n\n"
  
  if match_perc >= 50:
    msg += f"✨ **Afinidade Alta ({match_perc}%):** Essa vaga tem muito a sua cara! "
    if pontos_fortes:
      msg += f"Eles valorizam exatamente o que você já conhece em **{', '.join(pontos_fortes)}**. "
    msg += "Destaque suas experiências práticas e seu rigor técnico na inscrição.\n\n"
  else:
    msg += f"🌱 **Afinidade Base ({match_perc}%):** É uma ótima vaga para expandir horizontes! "
    msg += "No processo seletivo, mencione sua facilidade de aprendizado e adaptação rápida a protocolos.\n\n"
    
  msg += f"📍 **Dica Estratégica de Trajeto:** Fica em {vaga.location}. Planeje com antecedência a rota para não se cansar com trânsito no dia da entrevista!\n\n"
  msg += "💌 *'Você é extremamente competente e dedicada. Respira fundo, confia no seu potencial e vai com tudo. Eu acredito 100% em você!'* — Com amor, Thiago Zuza."
  return msg


# --- CSS COM ALTO CONTRASTE E TEMA HELLO KITTY ---
st.markdown(
    """
<style>
    .stApp {
...
with col_title:
  st.markdown(
      "<h1 style='color: #C2185B; margin-bottom: 0;'>Portal de Carreiras em"
      " Saúde & Biomedicina 💕</h1>",
      unsafe_allow_html=True,
  )
  st.markdown(
      "<p style='color: #880E4F; font-size: 1.05rem;'>Oportunidades em Belém e"
      " São Paulo com inteligência e cuidado para você 🌸</p>",
      unsafe_allow_html=True,
  )
...
# --- BARRA LATERAL: FILTROS REGIONAIS E PROFISSIONAIS ---
st.sidebar.markdown("### 🎀 Localização & Carreira")
filtro_estado = st.sidebar.radio(
    "Estado / Região:", ["Todos", "PA (Belém e Região)", "SP (São Paulo Capital)"]
)
filtro_categoria = st.sidebar.radio(
    "Área de Atuação:", ["Todas", "Enfermagem", "Biomedicina"]
)
busca_termo = st.sidebar.text_input(
    "🔍 Palavra-chave", placeholder="Ex: Einstein, Sírio, UTI, Coleta, Imunologia..."
)

st.sidebar.markdown("---")
st.sidebar.markdown("##### 🥠 Biscoito da Sorte Diário")
st.sidebar.info(
    "Seja no laboratório ou no hospital, seu olhar humano e atenção aos detalhes salvam vidas! Orgulho sem fim! 💕"
)
...
# Recupera perfil e keywords
with Session(engine) as session:
  perfil_user = session.exec(select(UserProfile)).first()
  user_kws = (
      [k.strip() for k in perfil_user.skills_keywords.split(",") if k.strip()]
      if perfil_user and perfil_user.skills_keywords
      else []
  )
...
with tab_vagas:
  st.markdown(
      f"<h4 style='color: #AD1457;'>🩺 Oportunidades Listadas:"
      f" <b>{len(vagas_lista)}</b></h4>",
      unsafe_allow_html=True,
  )
  ...
  if not vagas_lista:
    ...
    for v in vagas_lista:
      ...
      badge_estado = (
          '<span class="badge-sp">🏙️ São Paulo</span>'
          if v.state == "SP"
          else '<span class="badge">🌴 Belém - PA</span>'
      )
      badge_cat = (
          '<span class="badge-bio">🔬 Biomedicina</span>'
          if v.category == "Biomedicina"
          else '<span class="badge">🩺 Enfermagem</span>'
      )
      ...
      txt_zap = urllib.parse.quote(
          f"Olha essa vaga de {v.title} no {v.hospital_or_company} ({v.location}): {link_vaga}"
      )
      ...
      # Botão de IA Gemini + Hello Kitty
...
with tab_biomed:
  ...
  st.info(
      "Espaço dedicado a Análises Clínicas, Biologia Molecular, Imunologia e"
  ...
  if "Não" in tem_formacao:
    st.warning(
        "⚠️ **Atenção:** Vagas para *Biomédica Responsável Técnica*, emissão e"
        " assinatura de laudos exigem diploma e registro ativo no CRBM."
        " Enquanto não concluir, priorize vagas como **Técnica de Laboratório**,"
        " **Auxiliar de Coleta** ou **Estágio em Análises Clínicas**!"
    )
  else:
    st.success(
        "✨ **Elegível:** Você está apta a assumir bancadas analíticas, liberação"
        " de laudos e responsabilidade técnica laboratorial."
    )

  st.divider()

  # Modelo de Currículo Sugestivo e Carta
...
Estou à disposição para entrevista técnica e demonstração de competências de bancada.
